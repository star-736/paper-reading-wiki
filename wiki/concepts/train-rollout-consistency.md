...
把每个 token 的专家分配当成 rollout 数据的一部分随轨迹一起传，训练时**重放**而不是重算 top-k，让 token 精确地回到采样时处理它的专家。

- 由 Ma et al. 提出（arXiv:2510.11370，*Stabilizing MoE Reinforcement Learning by Aligning Training and Inference Routers*），[MiMo-V2-Flash](../sources/mimo-v2-flash.md) 的多 teacher OPD 用它稳定 MoE 路由（见 [Multi-Teacher On-Policy Distillation](multi-teacher-on-policy-distillation.md)）；[Miles](../sources/miles-v0-1.md) §2.5 把它做成 `--use-rollout-routing-replay`，因为 session server 已经记录 routed experts，重放覆盖整个多轮 episode 而非单次 completion。
- 代价是显存与带宽：每个 routing tensor 是 (tokens − 1) × layers × k 个 32 位整数，32K token × 60 层 × k = 8 约 **60 MB / 轨迹**，且必须随轨迹在内存里一起移动。因此它是 per-recipe 选择而非全局开关，dense 模型上是空操作。
...
- 这一层已有一张独立的方法谱系表，见 [LLM RL policy optimization 对比](../comparisons/llm-rl-policy-optimization.md) 的 KPop / IcePop / CISPO / DIS 各行——本页不重复：IcePop 用 uniform 固定区间 + double-sided masking，KPop 换成 binary KL divergence（因为低概率 token 的 ratio noise 更大，固定比率会过度 mask 它们），GLM-5 与 [SAO](../concepts/single-rollout-asynchronous-optimization.md) 的 DIS 用 $\pi_\theta/\pi_{\text{rollout}}$ 出界整段 mask。
...
- GLM-5.3 博客声称 `slime` 把「平均 log-prob 差异」控制到 $10^{-7}$ 量级，而建立在 slime 之上的 [Miles](../sources/miles-v0-1.md) 在 §9 报的是 train–inference KL 均值 **0.0369**。这两个数很可能不是同一个量（绝对 log-prob 差 vs per-token divergence 的均值），配置也不同（全数值对齐 vs BF16 训练 + FP8 服务），但仓库里没有能把两者对齐的第三方来源。这是本页最需要外部证据的一处。
- 图像 / 视频 token 还没有 TITO 级的精确性保证（Miles 的 session server 不携带视觉输入，VLM 只能直连 SGLang 低层接口），多模态 agentic RL 的 token 一致性靠什么保证？
- R3 的 routing tensor 成本随序列长度线性增长，1M 上下文下是否存在更低成本的等价机制？
- 「量化契约」与「ratio 修正」的边界应该是硬性的吗？Miles 的 NVFP4 例外说明有些格式无法只靠 ratio 修正兜底，但报告没给「哪些格式必须走契约、哪些可以靠修正」的判据。
- 各家的对齐声明几乎都是自报，缺少跨系统、同配置的独立测量。

## 相关页面

- 来源：[Miles v0.1](../sources/miles-v0-1.md)（五层手段的完整实现与限制清单）、[GLM-5](../sources/glm-5.md)（TITO + DIS + stale dropping）、[GLM-5.3 官方发布博客](../sources/glm-5-3-blog.md)（`slime` 数值对齐声明）、[Ling-2.6](../sources/ling-2.6.md)（module-aware FP8 + KPop）、[Laguna](../sources/laguna-m1-xs2.md)（chat-template 对齐断言 + FP8 KV cache 取舍）、[Kimi K3](../sources/kimi-k3.md)（QAT 贯穿 SFT 与 RL）
