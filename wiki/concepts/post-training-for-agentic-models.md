...
description: "面向 agent 的 RL、MOPD、蒸馏、LLM RL policy optimization、ARPO 这类 step-level rollout 采样、GiGPO/HGPO 这类 history-aware step 组 advantage、SAO 这类异步单 rollout，以及 Miles 这类把后训练当系统交付的实现报告。"
...
## MiniMax-M1：CISPO 与 hybrid 上的长生成 RL

[MiniMax-M1](../sources/minimax-m1.md)（2025-06）是 CISPO 的一手出处，也是后来 Laguna / Forge 引用的算法源头。它不改 GRPO 的组内相对优势，只改 clip **对象**：对 $r=\pi_\theta/\pi_{\theta_{\mathrm{old}}}$ 做 stop-grad 夹紧，所有 token 进梯度。原文不下 IS 下界、只调上界；$(c_{low},c_{high})=(1,4)$ 是 [Laguna](../sources/laguna-m1-xs2.md) 的采用超参，不是 M1 数字。

配套工程同样属于后训练：hybrid Lightning Attention 上 LM head 改 FP32 才把 train/infer 概率相关拉到 0.997；RL 先规则可验证、再混 GenRM 通用域；40K→80K 分窗扩，负样本顶窗时改用 sample-level + token-level 混用。完整 RL 三周 / 512 H800。MiniMax-M2 系列不是 CISPO 源头。
...
**2. CISPO + length-weighted LOO advantage**。RL 用 token-level REINFORCE surrogate + **CISPO** clipping [14]（[14]=[MiniMax-M1](../sources/minimax-m1.md)；MiniMax-M2 系列不是 CISPO 源头）+ length-weighted leave-one-out group-relative advantage。Laguna 的 asymmetric clipping $(c_{low},c_{high})=(1,4)$（有效 $[0,5]$）是采用方超参，M1 原文只写不下 IS 下界、只调上界。报告明说消融 vs [GRPO](../sources/deepseekmath.md) / [GSPO](../sources/group-sequence-policy-optimization.md) 后选 CISPO 是因「最终评测质量 + 训练稳定性组合最好」——这是 [LLM RL policy optimization 对比](../comparisons/llm-rl-policy-optimization.md) 里 CISPO 首次有公开的 vs GRPO/GSPO 消融选择理由。reward 设计只让 binary task verifier 给正 reward（1.0），其余全是小负惩罚（parsing/min-steps/tool-error per-token），长周期信用分配全靠末尾 verifier 经 advantage 传到每个 token。
...
## Miles：把「后训练系统」本身写成一份报告

[Miles v0.1](../sources/miles-v0-1.md)（RadixArk，建立在 `slime` 之上）与前几节不同——它不发布模型，而是把后训练当成一类**系统**来交付：rollout 侧绑定 SGLang，trainer 侧可选 Megatron-LM 或 PyTorch FSDP，中间是三种权重同步传输。值得单独记的是它的组织方式：

- **同一套组件服务五种范式**：全参 RL、LoRA RL、OPD、SFT、true-on-policy alignment，再加扩散模型（Miles-Diffusion）。共享的是 rollout engine / trainer / weight-update 路径，不是 RL 专属机制——这与 [Macaron-V1](../sources/macaron-v1.md) 的「unit 是权重、环境和 runtime contract 的联合作用」是同一判断的工程实现。
- **它把此前只有抽象名字的机制写细**：异步 staleness 的形式定义与三条件丢组、group vs sample 补位粒度、TITO session server 的 Linear / Branching 规则与可配置重放 matcher、R3 专家路由重放的成本、低精度契约的四阶段对齐、TIS / clip-or-pop。详见 [异步 Agent RL](asynchronous-agent-rl.md)、[训练—rollout 一致性](train-rollout-consistency.md) 与 [RL 权重同步与部署拓扑](rl-weight-synchronization.md)。
- **它把 OPD 放到框架侧**：不改算法（仍是一样本 reverse KL，仍引 Thinking Machines Lab 博客），而是把 divergence 折进 advantage，从而与任意 advantage estimator 组合，并给出 served / in-process teacher 两种部署。它的 Qwen3.5-35B-A3B 实验是已收录报告里少见的「蒸馏只把 response 缩到 56% 长度、accuracy 无可信变化」的诚实结论。
- **它的证据等级是显式的**：低精度只覆盖已测配置、9 个模型族的「supported」不等于同一等级、true-on-policy 只在 dense Qwen3 0.6B/4B 上有 profile、两个发布日 recipe 仍在 open PR、diffusion recipe 逐个标 fully gated / proxy gated / verified / not verified。§9 的单次 100 step 运行里，作者主动把 reward 曲线上升标成观察而非测得的改进。
...
[HGPO](../sources/hierarchy-of-groups-policy-optimization.md) 将这个假设再收紧一层：同一当前 state 不等于同一 effective prompt，因为 step-wise agent 会把有限历史留在 memory。它把 state group 按共同历史拆成嵌套 group，深层 bias 更低、样本也更少，再用深度权重聚合 advantage；仍不追加 rollout 或引入 critic。主证据限 ALFWorld / WebShop，且 7B 的少数单项不高于 GiGPO，因此应把它读作 history conditioning 这个维度的实证信号，不能读成已替代所有 state grouping。

[SAO](../sources/single-rollout-asynchronous-optimization.md) 问的是更前面的问题：异步长周期下还要不要组。它把 GRPO 的组采样换成 group size = 1，轨迹完成即训；没有组内相对奖励后把 critic 请回来，并用 DIS 把 $\pi_\theta/\pi_{\mathrm{rollout}}$ 出界的 token mask 掉。Qwen3-30B-A3B 上 vanilla GRPO 约 160 step 崩，SAO 报告约 1000 step；作者写已部署到 GLM-5.2 的 agentic RL pipeline。这和 GLM-5 报告里的 DIS 是同一层机制的算法论文，不要和 [SAPO](../sources/soft-adaptive-policy-optimization.md) 混名。

这补上了现有几条路线之间的空档：GLM-5 / Forge 关心 rollout 系统如何高吞吐、低偏差地跑起来；SAO 关心异步下组采样是否还成立，以及单条轨迹的 critic 怎么训；ARPO 关心 rollout 预算在一条 agent 轨迹内部应该投到哪里；GiGPO 关心同一组轨迹里如何用状态碰撞拆 step-level credit，HGPO 继续追问这些碰撞是否共享历史 context；VAPO 关心 value model 与 GAE 如何给 long-CoT 做 token-level credit assignment；DAPO / GSPO / SAPO 关心 value-model-free group-based RL 如何稳定、高效地转成 policy update。详见 [LLM RL policy optimization 对比](../comparisons/llm-rl-policy-optimization.md)。
...
- [DeepSeekMath](../sources/deepseekmath.md) / GRPO：如何丢掉 critic、用同题组内相对奖励当 baseline，把 PPO 收成 value-model-free 的 group-based 更新。后续 DAPO / GSPO / IcePop / KPop 都从这条目标改起。
...
- HGPO：如何在不追加 rollout 的前提下，进一步用共同历史控制同 state step group 的 prompt 混杂。
- SAO：如何在异步下放弃组采样，用 DIS + critic 让单条轨迹也能稳定更新。
...
- [UI-Mate](../sources/ui-mate.md)：如何把桌面 GUI RL 的决策单元和 reward 语义一起做对。两阶段 SFT → 在线 GRPO，不走 Xiaomi 的 Step RL 中间课。独有设计：(1) **Decision-turn centering + token 归一化**——GUI 失败轨迹更长、失败 turn 的 thinking 也更长，直接广播轨迹优势会偏向试错；(2) **可选 PCM** 把同一终局 `Ri` 重分配到 progress / causal error / recovery 步，作者报告它主要加快收敛而不是抬最终分；(3) **[IcePop](../sources/ring-1t.md) + SeqClip** 过滤异步 train–rollout mismatch，再叠加 PPO clip；(4) **DemoCUA SFT** 把示范条件化成第一种 user-turn 快照，并用 full-alignment / partial-misalignment / irrelevance 防止抄脚本。Historical reasoning 只在评测打开会涨分，写进 RL 训练则触发熵塌缩——与「保留 thinking」提升长周期任务的待追问直接相关。
...
- [ECHO](../sources/echo.md)（Microsoft Research，2026-05）：如何把 **已经在 rollout 里的环境观测**做成 dense 监督，而不另训 world model。在 GRPO 的同一 actor 前向上给终端输出 token 加 \(\lambda\mathcal{L}_{\mathrm{Env}}\)（只要 env 块、丢掉 warning 前缀；\(\lambda=0.05\)）。TerminalBench-2.0 pass@1：Qwen3-8B 2.70→5.17、14B 5.17→10.79。与 Unify（LWM warm-up）正交：ECHO 不换策略身份，只改哪些 token 进 loss；AgentWorld 原文把 compounding 列为 future work，ECHO 也没做。不要和 Kimi K3 里 MoE EP 的 ECHO/UltraEP 混名。

- [Miles v0.1](../sources/miles-v0-1.md)：如何把后训练的**系统**当成一等对象。同一套 rollout（SGLang）+ trainer（Megatron-LM / PyTorch FSDP）+ 权重同步路径同时服务全参 RL、LoRA RL、OPD、SFT、true-on-policy alignment 与扩散模型；报告把「吞吐」（affinity 路由、异步调度、staleness 治理）与「保真」（TITO、R3、量化契约、ratio 修正、bit-exact 对齐）拆成两条独立的工程轴，并把每条路径的覆盖范围与证据等级显式标出。与 [SAO](../sources/single-rollout-asynchronous-optimization.md) 互补：SAO 回答异步下「还要不要组采样」，Miles 回答异步下「staleness 怎么定义、buffer 什么时候丢组、评测怎么不挤占生成」。详见 [异步 Agent RL](asynchronous-agent-rl.md)、[训练—rollout 一致性](train-rollout-consistency.md)、[RL 权重同步与部署拓扑](rl-weight-synchronization.md)。

- [MMSearch-R1](../sources/mmsearch-r1.md)（ByteDance + NTU，2025-06）：首个端到端 RL 多模态按需搜索框架。GRPO 直接训练 Qwen2.5-VL-7B-Instruct（无 SFT cold-start），outcome-based reward + search penalty（0.1）塑造按需搜索行为。独有设计：(1) **两工具集成**——图像搜索（SerpApi 缩略图+标题）和文本搜索（SerpApi → Jina Reader → Qwen3-32B 摘要），图像搜索仅限第 1 轮；(2) **结构化 action tag 协议**——`<reason>` → `<search>` / `<text_search>` / `<answer>`，`<information>` 内容在 loss 时 mask；(3) **Search balancing**——FVQA-train 5K 样本均衡 search-required（~3400）和 search-free（~1600），防止模型总是搜索或从不搜索；(4) **搜索惩罚**——正确回答中执行了搜索则乘以惩罚因子，鼓励先用内部知识。5 benchmark 平均 54.6% 超同尺寸 RAG +3pp，搜索率降 32.9%，匹配 32B RAG 性能。RL 比 SFT（GPT-4o 蒸馏 8K）更高效（5K 样本即超过）。算法用标准 GRPO，不是新 policy optimization 方法。
- [DeepMMSearch-R1](../sources/deepmmsearch-r1.md)（Apple + JHU，2025-10）：[MMSearch-R1](../sources/mmsearch-r1.md) 的改进版，把后训练从"通用问答"转向"按需多轮 web search"。两阶段 SFT + GRPO，在 Qwen2.5-VL-7B-Instruct 上训练。独有设计：(1) **结构化工具调用协议**——用 XML-like tags（`<reason>`, `<text_search>`, `<img_search>`, `<answer>`, `<information>`）编码多轮搜索对话，`<information>` 在训练时被 mask，避免模型学习 web 噪声；(2) **裁剪图像搜索**——模型先生成 referring expression，Grounding DINO 裁剪相关区域，消除背景噪声，RL 后裁剪搜索 -37%（更精准，非更少）；(3) **DeepMMSearchVQA 数据管线**——InfoSeek → Gemini-2.5-Pro 生成多轮对话 → 正确性过滤 → 知识类别均匀采样，50:50 search-required:search-free 最优；(4) **RL 精炼工具行为**——SFT 学会工具使用，RL 减少不必要的裁剪搜索、增加多轮文本搜索（self-reflection +2.6%），使工具调用更高效。与 MMSearch-R1（单次调用、整图搜索）直接对标。算法用标准 GRPO，不是新 policy optimization 方法。
