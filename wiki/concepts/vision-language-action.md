...
description: "把预训练视觉语言模型接到机器人控制：看图、读指令、输出动作。动作头仍是三条：离散 token / π0 连续 flow / π0.5 开世界 co-training。FAST 是离散动作的压缩分词，不是第四种动作头。π0.7 仍停在连续 flow 上，但换成 Gemma 3 + MEM + 860M expert，不再是 PaliGemma + FAST→flow。AtomicVLA 是 SG-MoE 技能路由，不是第四种动作头。"
...
1. **离散动作 token 家族。** [RT-2](../models/rt-2.md) 是封闭定义：8 维（6-DoF + 夹爪 + terminate）、均匀 256-bin，与互联网数据 co-fine-tune，55B 闭环 1–3 Hz。 [OpenVLA](../models/openvla.md) 是开源复现：7 维、分位数 256-bin，覆盖进 Llama 词表，只在机器人数据上 fine-tune。两者动作头同类；OpenVLA 表里赢的是后续 **RT-2-X**，不是原版 RT-2。同一家族里，[FAST](../sources/fast.md) 改的是**分词**：把 1 秒 action chunk 做 DCT + 量化 + BPE，不是逐步 256-bin。它不是第四种动作头。
...
[π0.7](../models/pi0.7.md) **仍停在连续 flow 这一条上**，不是第四种动作头。骨干换成 **Gemma 3 4B + MEM**，expert 放大到 **860M**（§IV、Fig. 2）。训练时 VLM 仍用 [FAST](../sources/fast.md) token 做 Knowledge Insulation，但这不是 π0.5 的「FAST 预训练再切到 flow」两阶段。主张是把上下文写细（语言 / subgoal 图 / episode metadata），不是再换动作表示。不要臆造 π0.6 页。
...
[InternVLA-A1.5](../models/internvla-a1.5.md) 是再往后的 MoT + latent foresight 实例：Stage 1 仍用 [FAST](../sources/fast.md) 离散 token，Stage 2 起是 flow-matching 连续 chunk，并继续做 VQA / subtask；真机表拿 π0.5 当对照。
...
- **[FAST](../sources/fast.md)（2025，动作分词器，不是新 VLA）**：DCT + 量化 + BPE 把 **1 秒**动作 chunk 压成离散 token；相对 OpenVLA 式逐步 256-bin 改的是分词层，不是第四种动作头。FAST+ 是约 1M 真实轨迹上的 universal tokenizer。接到 π0 骨干上的 π0-FAST 匹配 diffusion VLA、训练最多 5× 更快——那是 FAST 论文的数字，不是 π0 原文。
- **[π0.5](../sources/pi0.5.md)（2025，开世界 co-training）**：骨干仍是 π0 的 PaliGemma + flow expert。改动是 [FAST](../sources/fast.md) 离散预训练再长 expert、**同一模型**先预测 semantic subtask，以及异构数据（MM 约 400 小时 / ~100 家 + ME/CE/HL/WD/VI）。评测在训练未见的厨房/卧室；97.6% 预训练样本不是目标平台家务（§I）。InternVLA-A1.5 真机表与 LIBERO-Plus 拿它当对照。
- **[InternVLA-A1.5](../sources/internvla-a1.5.md)（2026，MoT + latent foresight）**：不是把 OpenVLA 换个 backbone，也不是 π0.5 换 Qwen。VLM 是 Qwen-3.5 2B（3:1 GDN:full attention），旁边 460M unified expert 只在 full attention 层交互。Stage 1 用 [FAST](../sources/fast.md) 离散 token；Stage 2 起 flow-matching chunk（size 50）+ 从冻结 WAN2.2 蒸馏的 foresight token。语义侧继续 VQA / subtask。评测协议、数据（1.2M episodes + 3M 多模态）不可与 OpenVLA 附录 E 或 π0.5 家庭进度条直接横比。
...
- **[π0.7](../sources/pi0.7.md)（2026，可steer 的 flow VLA）**：不再是 PaliGemma + [FAST](../sources/fast.md)→flow 两阶段。骨干是 **Gemma 3 4B + MEM**，expert **860M**，总参约 5B。[FAST](../sources/fast.md) 只作 VLM 的 Knowledge Insulation 训练信号。开箱对标 π\*0.6 RL / π0.6 SFT specialist（Fig. 6）；跨本体衬衫折叠 π0.7 (GC) 进度 85.6% / 成功 80%，人类遥操 90.9% / 80.6%（§IX-C）。不要臆造 π0.6 页，也不要用这些柱图填 AtomicVLA / EmbodiedSkills 的表。
...
- 离散动作的压缩分词，不是第四种动作头：[FAST](../sources/fast.md)
- 连续 flow + action expert：[π0](../models/pi0.md) · [来源](../sources/pi0.md)
- 开世界 co-training：[π0.5](../models/pi0.5.md) · [来源](../sources/pi0.5.md)
- Gemma 3 + MEM + 更大 flow，不再是 PaliGemma + [FAST](../sources/fast.md)→flow：[π0.7](../models/pi0.7.md) · [来源](../sources/pi0.7.md)
