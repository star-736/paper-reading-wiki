...
- [UI-Mate](../sources/ui-mate.md) 把桌面 computer-use 的瓶颈再拆成两层：**可验证环境能否规模化**，以及**用户没写进指令的程序如何一次说清**。训练侧用任务–环境–capability tree–verifier 闭环喂 SFT 和在线 GRPO；交互侧把录屏蒸馏成 subtask workflow，live 截图保留否决权。这与 Xiaomi-GUI-0 互补而不是重复：Xiaomi 对齐移动真机的异常态分布，UI-Mate 对齐办公长工作流的程序歧义。OSWorkerBench 的成对协议把「一条示范的增益」从模型指令-only 能力里拆出来，但本报告只量化了同任务 self-demo，跨任务 variant-demo 尚未成为主结果。
- [Qwen-UI-Agent](../sources/qwen-ui-agent.md) 把 GUI agent 的工程瓶颈写成**跨域执行器**而不是单平台专精：同一策略覆盖真机手机、桌面 hybrid GUI+CLI、浏览器和 DeepSearch，再用 harness 回答「何时主动开始、工作流如何跨设备继续」。三处设计对应六条 sim-to-real 转移：(1) 约 10,000 并发沙箱 + 100+ 真机 / 虚拟屏，并把 model failure 与 environment failure 拆开；(2) 一次决策可发批动作，OSWorld-v2 上 CLI 已占 55.1% 动作；(3) AutoResearch 飞轮用 step-judge 扩 SFT、用可执行 verifier 做 Online RL，人只监督。与 Xiaomi（单域真机分布对齐）和 UI-Mate（示范程序）互补：它解决的是「一个 foundation GUI agent 如何同时在真机和桌面长程上跑，并且不只等用户开口」。Harness 工作流目前是定性演示，没有开关膜的成对实验。详见 [Qwen-UI-Agent 来源页](../sources/qwen-ui-agent.md)。
...
- [ECHO](../sources/echo.md) 把同一缺口收成更便宜的一条：不另训 LWM，只在 GRPO 里给终端观测 token 加辅助 CE。失败 rollout 从「零梯度」变成「环境如何响应」的监督；TB2 上 8B/14B 相对匹配 GRPO 近乎翻倍。它回答的是「观测已经在轨迹里，为什么不训」，不是「要不要一个独立模拟器」。与 AgentWorld 的 compounding 两边都没做。
- [Laguna](../sources/laguna-m1-xs2.md)（Poolside，2026-05）把 agentic engineering 的瓶颈定位在**模型开发流程本身**，而非模型规模或单点算法。核心论点是「把模型开发当工业流程而非手工艺」是 frontier 模型开发最 consequential 的杠杆——M.1 预训练结束后**五周**从零交付 XS.2 是这套 Model Factory 的实证。三条原则（experiments as code / composable decoupled components / reserve human attention for novel decisions）落到 agentic coding 上具体体现为：(1) 合成代码环境把真实 git commit 转成可验证任务（双端正确性检查 + repo 热度过滤），**同一可验证环境贯穿 SFT（teacher 轨迹）与 RL（per-repo test suite 作 binary verifier）**，复用率高于分别造数据；(2) RL 用 CISPO（来源：[MiniMax-M1](../sources/minimax-m1.md)；MiniMax-M2 系列不是 CISPO 源头；$(1,4)$ 是 Laguna 采用超参）+ length-weighted LOO advantage，明说消融 vs GRPO/GSPO 后选它；(3) IF judge + multi-harness 训练（OpenHands/OpenCode2/Mini-SWE-Agent）防 scaffold 过拟合。与 [KAT-Coder](../models/kat-coder.md)（训练基础设施稳定性）/ [daVinci-Agency](../sources/davinci-agency.md)（SFT 数据结构）/ [Kimi K3 Unified White-Box RL Env](../sources/kimi-k3.md)（harness-agnostic RL 配置）互补：Laguna 解决「整个开发流程如何可复用、可由非核心团队 self-service 跑起来」这一更外层的工程问题——XS.2 的初始 imitation learning 阶段由核心后训练团队之外的人独立跑完。
...
- [ECHO](../sources/echo.md) - 终端观测辅助 CE，不是独立 world model
...
- [UI-Mate](../models/ui-mate.md) - 桌面 CUA 数据飞轮 + DemoCUA 示范工作流
- [Qwen-UI-Agent](../models/qwen-ui-agent.md) - 跨域 foundation GUI agent + hybrid CLI + proactive harness
