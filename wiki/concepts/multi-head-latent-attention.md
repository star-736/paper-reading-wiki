...
- **现有材料待核**：MLA 各维度已据 V2 原文 §2.1.2–2.1.3 与配置表坐实主要值：KV latent $d_c=512$、query latent $d_c'=1536$、decoupled per-head $d_R=64$（$n_h=128,\ d_h=128$）；V2-Lite 则 $d_c=512$ 但**不压缩 query**。各上/下投影矩阵的完整形状仍可回原文配置表补全。
...
主记录：[MLA 与 MHA 的附录对照](../sources/deepseek-v2.md#待追问)。
