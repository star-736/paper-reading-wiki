...
[RWKV](../sources/rwkv.md)（Peng 等，EMNLP 2023）把 AFT 的 pairwise 偏置收成 channel-wise 衰减 $w$，WKV 是归一化的指数加权和。推理状态 $O(d)$，不是 $S_t\in\mathbb{R}^{d_k\times d_v}$。GLA 的 MQAR 里它弱于矩阵状态模型。RWKV-7 的 generalized delta 不在该 PDF。

**FlashLinearAttention 先是算法名，后来才是仓库伞名。** [GLA](../sources/gated-linear-attention.md) §3 把 I/O-aware chunkwise 线性注意力实现叫做 FLASHLINEARATTENTION。GitHub `flash-linear-attention` 后来把 Lightning / GLA / RetNet / GDN 收在一起，那是实现层。不要把仓库伞名写成第三种注意力。[Lightning Attention-2](../sources/lightning-attention-2.md) 的原文代码在 [OpenNLPLab/lightning-attention](https://github.com/OpenNLPLab/lightning-attention)。
...
- **[Gated Linear Attention](../sources/gated-linear-attention.md)（Yang 等，ICML 2024，原文已入库）**：矩阵线性注意力上 **数据相关 channel-wise 门** 的一手出处。$S_t=\mathrm{Diag}(\alpha_t)S_{t-1}+k^\top v$，没有 delta。KDA 的细门承自这里；GDN 走的是标量门 + delta，两条后作才在 KDA 会合。FlashLinearAttention 算法名出在本篇。
- **[RWKV](../sources/rwkv.md)（Peng 等，EMNLP 2023 Findings，原文已入库）**：AFT 收成 RNN 的旁支。WKV 是 channel-wise 指数衰减加权，推理状态 $O(d)$，不是 $d_k\times d_v$ 矩阵。GLA 的 MQAR 里 RWKV-4 弱于矩阵状态模型。不要把 RWKV-4 写成 GDN/KDA 前身；RWKV-5/6/7 不在该 PDF。
- **[MiniMax-M1](../sources/minimax-m1.md)（MiniMax，2025-06，456B / 45.9B reasoning）**：这条 **Lightning Attention 本族**上更早的开源生产混合栈。§1 原文：「a transformer block with softmax attention follows every seven transnormer blocks with lightning attention」——即 **7 Lightning : 1 softmax**，全局层是标准 softmax，**不是 MLA**。Lightning Attention 引用 Qin et al. 2024b，机制定义见 [Lightning Attention-2](../sources/lightning-attention-2.md)（标量衰减 tiling，不是 delta rule）。CISPO 回链不在本路写。
- **[Ling-2.6](../sources/ling-2.6.md)（Inclusion AI，2026-06，万亿参数 agentic）**：同一 Lightning 本族的后作混合栈，线性层仍是标量衰减递推 + tiling kernel，**不是** GDN/KDA 的 delta rule。混合比例同样 **7:1**，但全局层换成 **MLA**（7 Lightning Attention : 1 MLA），比 Kimi / Qwen 的 3:1 更激进。来源页由 A 路维护，IcePop 回链不在本路写。不要把 M1 的 softmax 全局层写成 Ling-2.6 的 MLA。
- 混合而非纯线性：纯线性注意力的有限状态做不好长程检索，所以 Kimi Linear 保留 1/4 的全局 [MLA](multi-head-latent-attention.md) 层、Qwen3-Next / 3.5 保留 1/4 的全局 gated attention 层、[Qwen3.8-Flash-Next](../models/qwen3.8-flash-next.md) 把那 1/4 在 CPT 换成 QSA、[MiniMax-M1](../models/minimax-m1.md) 保留 1/8 的全局 softmax 层、Ling-2.6 保留 1/8 的全局 MLA 层、**Kimi K3 保留 1/4 的 Gated MLA 层**维持全局信息流——这与「稀疏注意力在 MLA 上加 top-k」是两种不同的省法（一个换 token mixer，一个少看 token）。Qwen3.8 是同一混合骨架上再叠内容稀疏，两条省法同时用。混合比例本身成为新的架构超参：3:1（Kimi Linear/K3/Qwen 的 GDN/KDA 族）vs 7:1（Lightning 族：M1 接 softmax、Ling-2.6 接 MLA），选哪个取决于线性注意力变体的质量和模型规模。K3 在 3T 规模仍选 3:1，说明 KDA 升级（scaled sigmoid）提升了线性层质量但未到能再压低全局层比例的程度。
