...
![DeepSeek-V3.2 Figure 2：DSA 注意力架构。Input Hidden h_t 经下投影分成两路——左侧主路：c^Q 上投影出 per-head query（content q^A + decoupled RoPE q^R），c^{KV} 作为共享 latent KV；右侧绿色路径：Lightning Indexer 用少量 indexer head（q^I / k^I，partially apply RoPE）对所有历史 token 打分，Top-k Selector 据此选出 top-k 个 KV entry，只有这些被选中的 c^{KV} 进入 Multi-Query Attention core。绿色部分 = DSA 的稀疏选择路径。](../assets/deepseek-v32/fig2-dsa-architecture.png)

> Figure 2（原文截图，§ DSA 架构）：DSA 实例化在 MLA 的 MQA mode 下。绿色部分是 DSA 的选择路径——Lightning Indexer 打分 → Top-k Selector 筛选 → 仅 top-k KV entry 进入 core attention。
...
Indexer 的具体配置（GLM-5 附录 Table 10，GLM-4.5 对照）：

| 架构参数 | GLM-4.5（无 DSA） | GLM-5（DSA） |
| --- | --- | --- |
| # Attention Heads | 96 | 64 |
| # Key-Value Heads | 8 | –（走 MLA） |
| **# Indexer Attn Heads** | – | **32** |
| **# Indexer Head Dim** | – | **128** |
| KV LoRA Dim | – | 512 |

Table 10 是 GLM-5 全架构表，这里只摘 indexer 与 attention 相关行。GLM-4.5 列两格为「–」表示该架构无 indexer——indexer 是 DSA 新增结构，与 MLA 主分支并列存在，不参与 KV cache。
...
[YOIO / CLSA](../sources/yoio.md) 引用 IndexCache，但前提换成 YOCO 的一份全局 KV：结构上只留一个单头 indexer，16 层 cross-decoder 共用一次 token-level top-k。它证明的是「记忆已经共享时路由也应共享」，不是把 DSA 的 retention 从 1/4 再往下压。YOIO Figure 5 在 4B 栈上把 DSA 的未摊销 top-k 画得比 Transformer 还慢，与本页「indexer 自身 O(NL²)」一致；那张图不是 GLM-5 的端到端数字。
