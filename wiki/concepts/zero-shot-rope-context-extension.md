...
description: "不微调、只在推理时改位置映射，让 RoPE 模型用过训练窗：频率缩放（PI / NTK-aware / YaRN 频率切分 + attention temperature）对分组别名（Self-Extend / DCA / Jet-Long），以及 NoPE 这条避开问题的旁路。"
...
- **[Qwen3](../sources/qwen3.md)**：S3 把序列从 4K 拉到 32K，RoPE 基频 10k→1e6——按 YaRN 原文脚注，这是 **NTK-aware / ABF**，和推理期 YaRN 不是同一旋钮。公开推理配方是 **YaRN + Dual Chunk Attention，再 4×**，把有效长度扩到约 128K（1.7B 模型卡仍标 32K；4B/8B 标 128K，来源就是这套推理扩展，不是再训到 128K）。报告未写 $\alpha/\beta/t$，也未写 Dynamic Scaling。[Jet-Long](../sources/jet-long.md) 把同一 YaRN factor=4 当作基线：在 Qwen3-1.7B/4B/8B-Base 上 RULER 全面落后动态分组，1.7B 上 YaRN 甚至低于不扩展的 Base（52.99 vs 60.93）。读 Qwen3 的「128K 上下文」时，应把它当成 YaRN+DCA 的部署结果，而不是原生训练窗。
...
- 来源：[YaRN](../sources/yarn.md)（频率切分 + 温度的一手定义）、[Jet-Long](../sources/jet-long.md)（动态分组 vs 固定 factor YaRN）、[Qwen3 技术报告](../sources/qwen3.md)、[Qwen3-Next 官方博客](../sources/qwen3-next-blog.md)、[Laguna M.1/XS.2](../sources/laguna-m1-xs2.md)、[Kimi Linear](../sources/kimi-linear.md)、[Kimi K3](../sources/kimi-k3.md)
