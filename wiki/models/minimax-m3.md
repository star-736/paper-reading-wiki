...
| Long-context | 原生 1M-token 上下文窗口；后续约 140B tokens 用于长上下文扩展训练，HELMET-128K / RULER-128K 上接近 full attention baseline |
...
**参数规模**：根据 Hugging Face 和 GitHub 官方信息，MiniMax-M3 约 428B 总参数，约 23B 激活参数（MoE）。
...
MSA 论文（2024）对 MiniMax-M3 的实验版本（约 109B 总参 / 6B 激活）进行了预训练评测（MMLU、RULER、HELMET、AI2D、VideoMME、TemporalBench 等），反映稀疏注意力对预训练能力的影响。目前在 Hugging Face 上发布的生产版 MiniMax-M3 约 428B 总参 / 23B 激活，参数规模显著更大，其预训练和后训练数据可能有所不同。把 M3 与 M2.7 直接放进 [agentic 评测体系](../concepts/agentic-evaluation-benchmarks.md) 对比时需注意版本差异；后续如果发布 M3-instruct 或 agentic 后训练版本，再补充。
...
- HF 释出版本是否就是 MSA-PT 的 109B 模型，还是经过了进一步 post-training？根据现有信息，发布的 MiniMax-M3 是更大规模的版本（~428B 总参），因此可能不仅仅是 MSA-PT 的 109B 模型。需要进一步确认训练流程。
