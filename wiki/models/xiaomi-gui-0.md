...
- 与 [Qwen-UI-Agent](qwen-ui-agent.md) 同做移动真机，但对方把真机嵌进跨域 foundation agent（桌面 CLI、浏览器、DeepSearch + proactive harness）。RealMobile 100 任务 vs MobileWorld-Real 409 任务，协议和 judge 都不同，不能直接读成同一真机 SOTA。
...
- [Qwen-UI-Agent](qwen-ui-agent.md) - 跨域 foundation GUI agent + 真机对照
