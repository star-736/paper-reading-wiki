...
- **不存在「Qwen3.5 base 报告」可补**：截至录入，Qwen3.5 系列只发了本篇 Omni 报告（arXiv:2604.15804）+ HF 权重，没有独立的 base 技术报告。所以架构事实的 tier-1 来源就是 **HF config（权重本身）**，GDN + gated attention 的文字级设计动机散在 Gated Attention / GDN 两篇原论文和官方博客里——不要再去找一篇并不存在的 base 报告。
