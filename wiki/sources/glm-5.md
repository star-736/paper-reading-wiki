...
> on-policy cross-stage distillation 在 GLM-5 的定位与 MiMo MOPD / DeepSeek-V4 OPD（融合多个 domain teacher）不同——它的 teacher 是上游 RL checkpoint，目的是"swiftly recover the skills acquired in earlier SFT and RL stages"（§3.5 原文），缓解 sequential RL 阶段间的能力遗忘。详见 [On-Policy Distillation 跨报告对比](../comparisons/on-policy-distillation.md) 轴一第三类。
