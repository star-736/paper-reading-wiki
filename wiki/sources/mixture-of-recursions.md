...
- **需实验或作者披露**：把 KV 缓存写入／更新计入，并加入长 prompt 的 prefill 后，吞吐和单请求延迟分别改善多少？
- **需实验或作者披露**：Expert-choice 的训练 Top-K 与因果推理预测在分布外长序列上能否保持一致？2K 实验不能直接证明百万 token 收益。
- **需实验或作者披露**：在更大训练预算下，如何缓解共享参数容量瓶颈？1.7B 对照已经出现普通模型领先，不能仅凭小预算拟合线外推。
- **需实验或作者披露**：递归局部 attention 丢失深层历史 token，对检索、复制、长依赖及真实多步推理有什么影响？需要与首轮 KV 共享做任务级对照。
- **需实验或作者披露**：能否在保留因果推理与负载稳定性的同时，像 ITT 预算实验那样灵活改变参与比例？
