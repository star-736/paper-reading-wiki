...
- 与 [NSA](nsa.md)、[MoBA](moba.md) 一起属于"natively trained sparse attention"族，区别于 H2O / SnapKV / Quest / MInference / FlexPrefill 这类训练后稀疏化方法。MoBA 的门是块均值点积、每 head 独立、无额外 indexer；MSA 是独立 Index Branch + 每 GQA group。比较页上的 InfLLM-V2 行仍是二手、已标未核，不要从本页升级。
