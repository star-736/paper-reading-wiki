...
- 与 [NSA](nsa.md)、[MoBA](moba.md)、[InfLLM-V2](infllm-v2.md) 一起属于"natively trained sparse attention"族，区别于 H2O / SnapKV / Quest / MInference / FlexPrefill 这类训练后稀疏化方法。MoBA 的门是块均值点积、每 head 独立、无额外 indexer；MSA 是独立 Index Branch + 每 GQA group；InfLLM-V2 复用 dense KV、压缩只打分、短序列可切回 dense，需要 5B 长微调，不是零样本。
